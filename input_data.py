...
class ogbn_arxiv():
    def __init__(self):
        self.dataset = PygNodePropPredDataset(name='ogbn-arxiv',
                                                  root='/python code/gnn_ADMM_local/community_detection/dataset/',
        ...
        self.num_classes = self.y.size()[1]
        self.num_features = self.data.x.size()[1]
        self.product1 = azw(self.adj, self.x)
        self.product2 = azw(self.adj, self.product1)
        self.product3 = azw(self.adj, self.product2)
        self.product4 = azw(self.adj, self.product3)
        self.x = torch.cat((self.product4, self.product3, self.product2, self.product1, self.x), dim=1)
